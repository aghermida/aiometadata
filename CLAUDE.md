...
AIOMetadata is a personal fork of [`cedya77/aiometadata`](https://github.com/cedya77/aiometadata), a Stremio metadata addon (TMDB/TVDB-backed catalogs, search, AI-assisted search). This fork adds its own features on top (e.g. Ollama/self-hosted AI provider support, an admin auth gate, a landing page) while staying in sync with upstream.
...
2. **Prefer new, uniquely-named files/directories for fork-only functionality** (e.g. `addon/utils/ollama-service.ts`). Upstream will never create a file with that name, so there is no collision surface, ever — this is always safer than rule 1.
