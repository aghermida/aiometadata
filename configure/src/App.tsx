...
import { useForkGate } from './components/ForkGate';

function AppContent() {
  const { config } = useConfig();
  const isDashboardMode = !!(window as any).DASHBOARD_MODE;

  // [FORK-90003] landing page + admin auth wall — see ./components/ForkGate.tsx
  const forkGate = useForkGate(isDashboardMode);
  if (forkGate) return forkGate;
